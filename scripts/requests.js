...
const getLocation = async () => {
    const response = await fetch('https://ipinfo.io/json?token=03f9d39707a0bf')
    if (response.status === 200) {
...
const getLocationOld = () => {
    return fetch('https://ipinfo.io/json?token=03f9d39707a0bf').then(response => {
        if (response.status === 200) {
            return response.json()
        } else {
            throw new Error('Something is being missed!')
...
const getCurrentCountryOld = async () => {
    const responseOne = await fetch('https://ipinfo.io/json?token=03f9d39707a0bf')
    const responseTwo = await fetch('https://restcountries.com/v3.1/all')
